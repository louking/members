...
**Process lock**: Community commands that should not run concurrently (e.g. group sync, pending-review notifications) acquire a `fasteners.InterProcessLock` at the start and release it on completion, preventing cron overlap. Also needed so an overlapping run can't share the rate limiter's in-memory call budget with the one already running, since each process gets its own `_RateLimiter` instance with no cross-process coordination. All community commands share **one** lockfile, `COMMUNITY_LOCKFILE` (`community.py`, `/tmp/communitygroupmanager.lock`) — deliberately, not a per-command lockfile — since the thing actually being protected is Discourse's real server-side rate limit, which any two community commands share regardless of which ones they are. `CommunitySyncManager.start_import()` acquires/releases it manually; `community_review.check_pending_reviews()` wraps its whole body in `with InterProcessLock(COMMUNITY_LOCKFILE):`.
