...
## RunSignUp API Quirks

Found while building the non-binary awards workaround (#723 — `helpers.get_race_participants()`/`assign_race_divisions()`/`recalc_division_placements()`, `views/admin/awards_admin.py`'s `update_divisions()`/`update_event_awards()`). These are general RunSignUp API behaviors, not specific to that feature — relevant to any future code that touches divisions/results.

- **`supports_nb` (participants and results GET calls) is required to see a non-binary registrant's real gender.** Without it, both `GET /race/:id/participants` and `GET /race/:id/results/get-results` silently return `gender: null` for a registrant whose gender is `'X'` (non-binary) — indistinguishable from a registrant who simply left gender blank. Pass `supports_nb='T'`. Confirmed this is purely a visibility flag: it does not affect placement *computation*, only what the API exposes about a registrant's own gender field.
- **A division RunSignUp can't auto-place (most commonly non-binary — its own division-setup UI has no non-binary gender option) never gets `auto_selection_criteria` in `getracedivisions()`'s response at all**, not even a `gender: null` entry — the key is missing entirely. This is structurally indistinguishable from any other division an RD builds without auto-placement rules for unrelated reasons (e.g. a hand-curated combined award); there's no API signal to tell the two apart, only the division's own name text.
- **A division with *partial* auto-select criteria (e.g. age-only, no gender) is unsafe, not just incomplete.** It auto-matches *anyone* satisfying the given criteria regardless of gender. Confirmed live: a Male test registrant who wasn't already the winner of a higher-priority division won an age-only "non-binary" division outright, displacing the actual non-binary entrant — and because that counted as a "prior win," he was then also excluded from his own correct gendered age-group award, which cascaded to the wrong (slower) runner. Re-ordering division priority does not fix this — it only changes who's excluded from what via the win-cascade, it can't stop a division with real (even partial) criteria from matching anyone who satisfies them. The only safe configuration for a division RunSignUp can't properly categorize is *no* criteria at all, with membership handled explicitly (next point).
- **`POST /race/:id/divisions/assign-divisions` is the only way to give a no-criteria division real membership, and it's API-only** — there is no RunSignUp UI screen for a human to manually assign a registrant to a division. It only works on divisions that have no auto-select criteria; RunSignUp rejects it otherwise. It's meant for a third-party integrator (this club's other timing vendor, RaceDay Scoring, uses it this way).
- **`assign-divisions` completely *replaces* a registrant's manual-division memberships — it does not add to them.** Sending `race_division_ids: [B]` for a registrant already correctly placed in no-criteria division A silently wipes out their A membership if A isn't also included in the same call. This bit `members`' own code directly: `_nonbinary_assignments()` originally only listed the division that still needed a placement, not every division the registrant was eligible for, which dropped an already-correct membership the moment the registrant became eligible for a second flagged division. Fix: always send the registrant's *complete* current set of eligible divisions in one call, never just the newly-needed one(s). Sending an empty `race_division_ids` list for a registrant removes all their manual assignments.
- **Neither changing/removing a division's `auto_selection_criteria` nor clearing a registrant's `assign-divisions` membership retroactively clears or recomputes previously-computed placements.** A stale placement value — possibly wrong, e.g. from when a division had leaky partial criteria — persists until an explicit `POST /race/:id/results/recalc-division-placements` call, no matter what changes underneath it. Confirmed for both directions: fixing a division's criteria, and unassigning a registrant.
- **`assign-divisions` membership is stored independently of any result set and survives a result set being deleted.** Deleting a result set (RunSignUp's "Manage Result Sets" page, or the equivalent API call) only clears computed placement values, not manual division assignments. Posting fresh results into a brand-new result set auto-recomputes placements for anyone still assigned, with no new `assign-divisions` call needed. To get a registrant back to a genuine "never assigned" state, you must explicitly call `assign-divisions` with an empty list for them, *and* then call `recalc-division-placements` (per the point above) to actually clear the cached value — the empty-list call alone leaves the stale placement in place.
- **A plain results POST (`results/full-results`) auto-triggers recomputation for an auto-select division immediately, and also for a no-criteria division *once it already has explicit `assign-divisions` membership* — but the very first time, an explicit `recalc-division-placements` call is required after `assign-divisions`.** Assignment alone does not trigger computation. After that initial bootstrap, ordinary results corrections (a timing fix, a new results upload) keep a manually-assigned division current automatically, same as any other division.
- **Division priority/exclusivity is computed identically for a manually-assigned division and an auto-select one** — no separate logic needed to account for it. RunSignUp's `recalc-division-placements` uses each division's own `priority` field regardless of how its membership was established, including the exclusivity rule (a winner of a higher-priority division doesn't also win a lower-priority one).
...
- **A category's review-moderator group(s) are not on `GET /categories.json`.** There is no `reviewable_by_group_name`-style field despite that being the more commonly-referenced Discourse feature name for this. Use `GET /c/{id}/show.json` instead — `category.moderating_group_ids` (an array of **group ids**; a category can have more than one moderator group — confirmed live, FSRC's `public-calendar-events` has two). Resolve ids to names via `GET /groups.json` (`fetch_groups()` in `community_taxonomy.py`). Used by `community_review.fetch_category_moderator_groups()`.

  **`category.topic_posting_review_group_ids`/`category.reply_posting_review_group_ids` are a different field and must NOT be used for this** — confirmed live (github.com/louking/members#721) they back the category's separate "Groups that do not require approval" exemption lists (New topic/reply approval settings on the same Moderation admin tab), not "who reviews this category." An earlier version of `fetch_category_moderator_groups()` read these by mistake; it looked correct because an operator had also added the real moderator groups to the exemption lists, until the exemption list also picked up an exemption-only group with "Who can message this group" set to Nobody — which broke the *entire* notice PM (Discourse fails the whole send if even one `target_recipients` entry is unmessageable), not just that one group's notification.
